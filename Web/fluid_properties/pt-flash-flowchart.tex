...
\node[decision] at (0,-20mm) (d_env) {Closed phase\\envelope built?};
\draw[arr] (entry) -- (d_env);

% Envelope-guided fast path: a previously built, closed phase envelope is used
% to classify (T,P) directly via is_inside(), bypassing Stage-1 stability.
\node[process, minimum width=40mm, text width=38mm]
  at (-54mm,-20mm) (env_guided)
  {Envelope-guided\\classification (\texttt{is\_inside})};
\draw[arr] (d_env.west) -- node[annot, above] {yes} (env_guided);

\node[terminal=navy, minimum width=22mm, text width=20mm]
  at (-54mm,-62mm) (env_single) {Single-phase\\density solve};
\draw[arr] (env_guided.south) -- node[annot, right, pos=0.3] {outside} (env_single);
...
\node[iterbox] at (176mm, -168mm) (gibbs)
  {Gibbs energy minimisation\\[-0.3mm]
   scaled Newton + Hebden trust-region\\[-0.3mm]
   ($v_i\!=\!\beta\,y_i$ variables)};

\node[decision] at (176mm, -190mm) (d_gate) {Equal-fugacity\\converged?};

\node[process, minimum width=38mm, text width=36mm]
  at (176mm, -212mm) (final_update)
...
\draw[arr] (gibbs) -- (d_gate);
\draw[arr] (d_gate.south) -- node[annot, right, pos=0.4] {yes} (final_update);

% Yes bypass: d_ss2 → final_update (route right of spine)
\draw[arr] (d_ss2.east) -- node[annot, above] {yes} ++(14mm,0)
  |- (final_update.east);

% Convergence gate (#3168): a non-converged split is never published — the
% solver raises, and the blind flash then falls back to a single-phase solve.
\node[terminal=amber!80!black, minimum width=24mm, text width=22mm]
  at (130mm, -190mm) (gate_fail) {raise $\to$\\single-phase\\fallback};
\draw[arr] (d_gate.west) -- node[annot, above, pos=0.5]
  {\textcolor{amber!80!black}{$>10^{-7}$}} (gate_fail);

% Result classification
\node[decision, minimum width=30mm, text width=20mm]
  at (176mm, -234mm) (d_beta) {$\beta$ value?};
\draw[arr] (final_update) -- (d_beta);

\node[terminal=navy, minimum width=20mm, text width=18mm]
  at (154mm, -254mm) (res_liq) {Liquid\\$\beta \approx 0$};
\node[terminal=cred, minimum width=20mm, text width=18mm]
  at (176mm, -254mm) (res_2ph) {Two-phase\\$0\!<\!\beta\!<\!1$};
\node[terminal=navy, minimum width=20mm, text width=18mm]
  at (198mm, -254mm) (res_vap) {Vapour\\$\beta \approx 1$};
...
% Envelope fast path → Stage 2 (interior points bypass Stage-1 stability).
% Routes over the top of the diagram and drops into the same merge as the
% blind "unstable" arrow, so both reach the phase split.
\coordinate (env_top) at (-54mm, 14mm);
\coordinate (merge_top) at (132mm, 14mm);
\draw[darr, color=cred, line width=0.7pt]
  (env_guided.north) -- (env_top) -- (merge_top) -- (s2merge);
\node[annot, text=cred, anchor=south] at (44mm, 14mm)
  {envelope interior $\Rightarrow$ seed phase split (bypass stability)};
...
\begin{scope}[on background layer]
  ...
  % Stage 2 box
  \node[fit=(rr)(ss2)(d_ss2)(gibbs)(d_gate)(final_update),
